...
public sealed class TreeGLControl : UserControl
{
    ...
    public void SetTree(ITree? tree, TreeMesh? mesh)
    {
        if (InvokeRequired) { Invoke(() => SetTree(tree, mesh)); return; }
    ...
    public void SetMesh(TreeMesh? mesh) => SetTree(null, mesh);
    ...
    private void DoGLInit()
    {
        ...
        GL.Light(LightName.Light0, LightParameter.Diffuse,  new[] { 1f, 0.95f, 0.85f, 1f });
        GL.Light(LightName.Light0, LightParameter.Ambient,  new[] { 0.28f, 0.28f, 0.28f, 1f });
